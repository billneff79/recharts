...
describe('<ReferenceLine />', () => {
  ...
  it('Should pass through className property to outermost wrapper', () => {
    const Label = ({ text, ...props }) => <text {...props} >{text}</text>;
    const wrapper = render(
		<BarChart width={1100} height={250} barGap={2} barSize={6} data={data} margin={{ top: 20, right: 60, bottom: 0, left: 20 }}>
			<XAxis dataKey="name" />
			<YAxis tickCount={7} />
			<Bar dataKey="uv" />
			<ReferenceLine className="customClassName" y={20} stroke="#666" label={<Label text="Custom Text" />} alwaysShow />
		</BarChart>
	);
    expect(wrapper.find('.recharts-reference-line.customClassName').length).to.equal(1);
  });
});
